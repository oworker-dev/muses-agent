...
`@oworker/open-agent-client/eve-session` is the default Eve 0.31.x adapter for the
host-neutral AgentSession cursor, streaming, stable-ID follow-ups, cancellation,
and reset interfaces. Eve is an optional peer dependency so Headless AgentRun
hosts do not install a Harness client they do not use.
