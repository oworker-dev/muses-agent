...
const adapterConfigSchema = z.object({
    ...
    broker: z.object({
        getServiceToken: z.custom((value) => typeof value === "function", "must be a runtime service-token resolver"),
...
export function createBrokeredMcpConnection(input) {
    const config = adapterConfigSchema.parse(input);
    return defineMcpClientConnection({
        approval: ({ toolName }) => approvalForMcpTool(toolName, config.connection.tools.requireApproval),
        auth: (ctx) => {
...
export function authorizeMcpSession(ctx, adapter) {
    const current = ctx.session.auth.current;
    const initiator = ctx.session.auth.initiator;
    const principal = current ?? initiator;
    if (!principal)
        throw new Error("An authenticated Agent session is required for MCP access.");
    ...
    if (!policy.mcpConnections.some((connection) => connection.id === adapter.id && connection.version === adapter.version)) {
    ...
    return {
        ...
        subject: {
            actorType: boundedText(textAttribute(principal.attributes.actorType) ?? principal.principalType, "MCP actor type", MAX_SUBJECT_ATTRIBUTE_BYTES),
            principalId: boundedText(principal.principalId, "MCP principal id", MAX_SUBJECT_ATTRIBUTE_BYTES),
            tenantId: boundedText(tenantId, "MCP tenant id", MAX_SUBJECT_ATTRIBUTE_BYTES),
        },
    };
}
export function approvalForMcpTool(qualifiedToolName, toolsRequiringApproval) {
...
export async function resolveBrokeredCredential(input) {
    ...
    try {
        ...
        if (error instanceof ConnectionAuthorizationFailedError)
            throw error;
        throw unavailable(input.request.adapter.id, "credential_broker_unavailable", true);
    }
    if (!response.ok) {
        throw unavailable(input.request.adapter.id, response.status === 401 || response.status === 403
            ? "credential_revoked_or_forbidden"
            : "credential_broker_unavailable", response.status >= 500);
    }
    let parsed;
    try {
...
function parsePolicy(value) {
    ...
    const result = z.array(adapterRefSchema).max(MAX_POLICY_CONNECTIONS).safeParse(parsed.mcpConnections);
    if (!result.success)
        throw new Error("The AgentRun MCP policy is invalid.");
    return { mcpConnections: result.data };
}
async function readBoundedBody(response, connection) {
    if (!response.body)
        return "";
...
function unavailable(connection, reason, retryable) {
