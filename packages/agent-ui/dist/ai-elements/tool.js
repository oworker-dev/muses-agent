...
export const ToolHeader = ({ className, title, type, state, showStatus = true, statusLabel, toolName, ...props }) => {
    const derivedName = type === "dynamic-tool" ? toolName : type.split("-").slice(1).join("-");
    return (_jsxs(CollapsibleTrigger, { className: cn("flex w-full items-center justify-between gap-4 p-3", className), ...props, children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(WrenchIcon, { className: "size-4 text-muted-foreground" }), _jsx("span", { className: "font-medium text-sm", children: title ?? derivedName }), showStatus ? getStatusBadge(state, statusLabel) : null] }), _jsx(ChevronDownIcon, { className: "size-4 text-muted-foreground transition-transform group-data-[state=open]:rotate-180" })] }));
};
...
export const ToolInput = ({ className, input, label = "Parameters", ...props }) => (_jsxs("div", { className: cn("space-y-2 overflow-hidden", className), ...props, children: [_jsx("h4", { className: "font-medium text-muted-foreground text-xs", children: label }), _jsx("div", { className: "rounded-md bg-muted/50", children: _jsx(CodeBlock, { code: JSON.stringify(input, null, 2), language: "json" }) })] }));
export const ToolOutput = ({ className, output, errorLabel = "Error", errorText, resultLabel = "Result", ...props }) => {
    ...
    return (_jsxs("div", { className: cn("space-y-2", className), ...props, children: [_jsx("h4", { className: "font-medium text-muted-foreground text-xs", children: errorText ? errorLabel : resultLabel }), _jsxs("div", { className: cn("overflow-x-auto rounded-md text-xs [&_table]:w-full", errorText ? "bg-destructive/10 text-destructive" : "bg-muted/50 text-foreground"), children: [errorText && _jsx("div", { children: errorText }), Output] })] }));
};
