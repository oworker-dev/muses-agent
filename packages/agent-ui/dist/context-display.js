...
function ContextDisplayRootBase({ modelContextWindow, children, labels: labelsOverride, usage, }) {
...
function RingVisual() {
    ...
    return (_jsxs("svg", { "aria-hidden": "true", width: RING_SIZE, height: RING_SIZE, viewBox: `0 0 ${RING_SIZE} ${RING_SIZE}`, className: "-rotate-90", children: [_jsx("circle", { cx: RING_SIZE / 2, cy: RING_SIZE / 2, r: RING_RADIUS, fill: "none", strokeWidth: RING_STROKE, className: "stroke-muted" }), _jsx("circle", { cx: RING_SIZE / 2, cy: RING_SIZE / 2, r: RING_RADIUS, fill: "none", strokeWidth: RING_STROKE, strokeLinecap: "round", strokeDasharray: RING_CIRCUMFERENCE, strokeDashoffset: RING_CIRCUMFERENCE - (percent / 100) * RING_CIRCUMFERENCE, className: cn("transition-[stroke-dashoffset,stroke] duration-300", getStrokeColor(percent)) })] }));
}
function RingPercentLabel() {
    const { percent } = useContextDisplay();
    return _jsxs("span", { className: "font-mono tabular-nums", children: [Math.round(percent), "%"] });
}
const ContextDisplayRing = ({ modelContextWindow, className, label, labels, side, usage, }) => (_jsxs(ContextDisplayRoot, { labels: labels, modelContextWindow: modelContextWindow, usage: usage, children: [_jsxs(ContextDisplayTrigger, { className: cn("text-muted-foreground hover:text-foreground gap-1.5 px-1.5 py-1 text-xs", className), "aria-label": label ?? "Context usage", children: [_jsx(RingVisual, {}), _jsx(RingPercentLabel, {})] }), _jsx(ContextDisplayContent, { side: side })] }));
function BarVisual() {
...
const ContextDisplayBar = ({ modelContextWindow, className, label, labels, side, usage, }) => (_jsxs(ContextDisplayRoot, { labels: labels, modelContextWindow: modelContextWindow, usage: usage, children: [_jsx(ContextDisplayTrigger, { className: cn("px-2 py-1", className), "aria-label": label ?? "Context usage", children: _jsx(BarVisual, {}) }), _jsx(ContextDisplayContent, { side: side })] }));
function TextVisual() {
...
const ContextDisplayText = ({ modelContextWindow, className, label, labels, side, usage, }) => (_jsxs(ContextDisplayRoot, { labels: labels, modelContextWindow: modelContextWindow, usage: usage, children: [_jsx(ContextDisplayTrigger, { "aria-label": label ?? "Context usage", className: cn("text-muted-foreground hover:bg-accent hover:text-accent-foreground px-2 py-1 font-mono text-xs tabular-nums", className), children: _jsx(TextVisual, {}) }), _jsx(ContextDisplayContent, { side: side })] }));
