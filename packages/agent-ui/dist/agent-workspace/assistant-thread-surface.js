...
import { ArrowDownIcon, ArrowUpIcon, AtSignIcon, CheckIcon, CopyIcon, LoaderCircleIcon, PaperclipIcon, PencilIcon, ShieldCheckIcon, SlashIcon, SquareIcon, WrenchIcon, } from "lucide-react";
    ...
    return (_jsx(ComposerPrimitive.Unstable_TriggerPopoverRoot, { children: _jsxs(ComposerPrimitive.Root, { className: "relative flex w-full flex-col", onSubmit: (event) => {
                ...
                if (!composerDisabled)
                    aui.composer.send();
                                ...
                                event.preventDefault();
                                aui.composer.send();
                            }, className: "aui-composer-input relative max-h-40 min-h-12 w-full resize-none overflow-y-auto bg-transparent px-2 py-1 text-[15px] leading-6 outline-none aria-disabled:pointer-events-none aria-disabled:opacity-50 [&_.aui-directive-chip]:inline-flex [&_.aui-directive-chip]:items-center [&_.aui-directive-chip]:gap-1 [&_.aui-directive-chip]:rounded-md [&_.aui-directive-chip]:bg-muted [&_.aui-directive-chip]:px-1.5 [&_.aui-directive-chip]:py-0.5 [&_.aui-directive-chip]:text-[13px] [&_.aui-directive-chip]:font-medium [&_.aui-directive-chip]:text-foreground [&_.aui-directive-chip-icon]:text-muted-foreground [&_.aui-lexical-input]:min-h-6 [&_.aui-lexical-input]:outline-none [&_.aui-lexical-placeholder]:pointer-events-none [&_.aui-lexical-placeholder]:absolute [&_.aui-lexical-placeholder]:inset-x-0 [&_.aui-lexical-placeholder]:top-0 [&_.aui-lexical-placeholder]:truncate [&_.aui-lexical-placeholder]:px-2 [&_.aui-lexical-placeholder]:py-1 [&_.aui-lexical-placeholder]:text-muted-foreground" }), _jsxs("div", { className: "flex min-h-8 items-center gap-1", children: [_jsx(ComposerPrimitive.AddAttachment, { asChild: true, children: _jsx(Button, { "aria-label": messages.addFiles, className: "size-8 rounded-full text-muted-foreground", size: "icon-sm", type: "button", variant: "ghost", children: _jsx(PaperclipIcon, { className: "size-4" }) }) }), _jsx(ExecutionModeMenu, { messages: messages, onChange: (executionMode) => onPreferencesChange({ ...preferences, executionMode }), value: preferences.executionMode ?? "standard" }), _jsx(ModelSelector, { align: "start", className: "h-8 max-w-56 rounded-full text-muted-foreground", contentClassName: "w-72", effort: preferences.reasoning, effortLabel: messages.reasoning, models: selectorModels, onEffortChange: (reasoning) => onPreferencesChange({ ...preferences, reasoning }), onValueChange: (modelId) => onPreferencesChange({ ...preferences, modelId }), searchable: models.length > 6, size: "sm", value: model?.id ?? preferences.modelId, variant: "ghost", triggerLabel: messages.model }), _jsxs("span", { className: "ml-auto flex items-center gap-1", children: [model ? (_jsx(ContextDisplay.Ring, { className: "h-8 rounded-full px-1.5", label: messages.context, labels: contextLabels, modelContextWindow: model.contextWindowTokens, side: "top", sessionUsage: sessionUsage, usage: contextUsage })) : null, stopping || (isRunning && composerIsEmpty) ? (_jsx(ComposerPrimitive.Cancel, { asChild: true, children: _jsx(Button, { "aria-label": cancellationState === "idle" ? messages.cancel : messages.stopping, className: "size-8 rounded-full", disabled: cancellationState !== "idle", size: "icon-sm", type: "button", children: cancellationState === "idle" ? (_jsx(SquareIcon, { className: "size-3.5 fill-current" })) : (_jsx(LoaderCircleIcon, { className: "size-4 animate-spin" })) }) })) : (_jsx(Button, { "aria-label": isRunning ? messages.queueFollowUp : messages.send, className: "size-8 rounded-full", disabled: composerDisabled, onClick: () => aui.composer.send(), size: "icon-sm", type: "button", children: _jsx(ArrowUpIcon, { className: "size-4" }) }))] })] })] }), _jsx(ComposerTriggerPopover, { char: "@", ...mention, emptyItemsLabel: messages.noPromptItems }), _jsx(ComposerTriggerPopover, { char: "/", ...command, emptyItemsLabel: messages.noPromptItems })] }) }));
}
